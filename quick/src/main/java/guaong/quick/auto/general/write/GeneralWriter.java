package guaong.quick.auto.general.write;

import guaong.quick.auto.core.resovle.bean.ConfigInfo;
import guaong.quick.auto.core.exception.FileException;
import guaong.quick.auto.core.util.StringUtil;
import guaong.quick.auto.core.exception.WriteFailException;
...
public class GeneralWriter {
    ...
    public GeneralWriter(Map<Integer, String> generalMap, ConfigInfo configInfo){
        this.generalMap = generalMap;
        this.configInfo = configInfo;
    }

    public boolean write(){
        if (!createFile(configInfo.getExportUrl())) return false;
        Set<Integer> set = this.generalMap.keySet();
        String tableName = configInfo.getTableName();
        set.forEach(i -> {
            String url = configInfo.getExportUrl() + chooseFileName(i, tableName);
